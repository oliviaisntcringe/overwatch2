	...
	inline void overlay_thread() {
		__try {
			...
			while (FindWindowA(skCrypt("TankWindowClass"), NULL))
			{
				...
				ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.f / 255.f, 0.f / 255.f, 0.f / 255.f, 70.f / 255.f));
				ImGui::RenderNotifications();
				ImGui::PopStyleVar(1);
				ImGui::PopStyleColor(1);
				ImGui::SetNextWindowPos(ImVec2(0, 0)); // Set the window position to the top-left corner
				ImGui::SetNextWindowSize(ImVec2(200, 40)); // Set the window size to 200x50

				ImGuiWindowFlags flags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoTitleBar;
				ImGui::PushStyleVar(ImGuiStyleVar_Alpha, 0.7f); // Set window transparency to 0.7
				ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0, 0, 0, 1)); // Set window background color to black

				ImGui::Begin("My Window", nullptr, flags); // Begin the window with the title "My Window"
				ImGui::Text("TRIADA | public edition "); // Display the text inside the window
				ImGui::End(); // End the window

				ImGui::PopStyleVar(); // Restore default alpha value
				ImGui::PopStyleColor(); // Restore default window background color
				if (Config::Menu)
				{
					static float animationProgress = 0.0f; // animation progress (0.0f to 1.0f)
					static std::vector<ImVec2> circlePositions; // store circle positions
					static std::vector<float> circleSizes; // store circle sizes
					static std::vector<float> circleSpeeds; // store circle speeds

					// Initialize circle positions, sizes, and speeds
					circlePositions.push_back(ImVec2((float)rand() / RAND_MAX * ImGui::GetIO().DisplaySize.x, 0.0f)); // initial circle position
					circleSizes.push_back((float)rand() / RAND_MAX * 2.0f + 2.0f); // random size between 2.0f and 12.0f
					circleSpeeds.push_back((float)rand() / RAND_MAX * 50.0f + 10.0f); // random speed between 10.0f and 60.0f

					ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0, 0, 0, 0.7f)); // Set window background color to black with 0.7f alpha
					ImGui::Begin("Backdrop", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoSavedSettings);
					ImGui::SetWindowPos(ImVec2(0, 0)); // Set window position to top-left corner
					ImGui::SetWindowSize(ImGui::GetIO().DisplaySize); // Set window size to full screen
					...
					for (int i = 0; i < circlePositions.size(); i++) {
						circlePositions[i].y += circleSpeeds[i] * ImGui::GetIO().DeltaTime; // move circle down
						if (circlePositions[i].y > ImGui::GetIO().DisplaySize.y) {
							circlePositions.erase(circlePositions.begin() + i); // remove circle when it reaches the bottom
							circleSizes.erase(circleSizes.begin() + i);
							circleSpeeds.erase(circleSpeeds.begin() + i);
							i--; // adjust index
						}
						else {
							ImGui::GetWindowDrawList()->AddCircle(circlePositions[i], circleSizes[i], IM_COL32(255, 255, 255, 255)); // draw circle
						}
					}

					// Spawn new circles at the top
					if (ImGui::GetIO().DeltaTime > 0.01f) { // spawn a new circle every 0.01 seconds
						circlePositions.push_back(ImVec2((float)rand() / RAND_MAX * ImGui::GetIO().DisplaySize.x, 0.0f));
						circleSizes.push_back((float)rand() / RAND_MAX * 10.0f + 2.0f);
						circleSpeeds.push_back((float)rand() / RAND_MAX * 50.0f + 10.0f);
					}
					ImVec2 boxPos = ImVec2((ImGui::GetIO().DisplaySize.x - 850) / 2, 0); // calculate center position horizontally, and top vertically
					ImGui::GetWindowDrawList()->AddRectFilled(boxPos, ImVec2(boxPos.x + 850, boxPos.y + 110), IM_COL32(0, 0, 0, 178)); // draw filled rectangle with a height of 110 pixels and 0.7 transparency

					// Calculate the center position of the box
					ImVec2 centerX = ImVec2(boxPos.x + 850 / 2, boxPos.y + 110 / 2); // calculate center position horizontally and vertically

					// Calculate the text size
					ImVec2 textSize = ImGui::CalcTextSize("TRIADA");

					// Calculate the text position, taking into account the text size and adjusting a little bit to the left and upper
					ImVec2 textPos = ImVec2(centerX.x - textSize.x / 2  -85,centerX.y - textSize.y / 2 - 35);

					// Set the text color to white and font size to 52 pixels
					ImGui::GetIO().FontGlobalScale = 4.0f; // increase font size to 52 pixels
					ImGui::PushFont(ImGui::GetIO().Fonts->Fonts[0]); // switch to a monospace font
					ImGui::GetWindowDrawList()->AddText(textPos, IM_COL32(255, 255, 255, 255), "TRIADA");
					ImGui::PopFont();
					ImGui::GetIO().FontGlobalScale = 1.0f; // reset font size
					...
					ImGui::SetNextWindowPos(ImVec2(OW::WX / 2.f - 550.f, OW::WY / 2.f - 410.f));
					ImGui::SetNextWindowSize(ImVec2(1100, 850));
					bool _visible = true;
					// Create a new window named "MAIN" with a size of 850x400

					if (ImGui::Begin(skCrypt(u8"TRIADA"), &_visible, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoTitleBar)) {
						...
						if (tab_index == 1) {
							ImGui::Spacing();
							ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255.f / 255.f, 0.f / 255.f, 0.f / 255.f, 70.f / 255.f));
